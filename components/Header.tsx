...
const Header: React.FC<HeaderProps> = ({ onCartClick, onWishlistClick, searchQuery, onSearchChange }) => {
  ...
  return (
    <>
      ...
      {/* --- Mobile Menu Overlay --- */}
      <div className={`fixed inset-0 top-[65px] bg-[--color-bg]/95 backdrop-blur-lg z-40 md:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          {isMobileMenuOpen && (
              <div className="container mx-auto px-6 py-8 flex flex-col items-center space-y-8 animate-fade-in-up">
                  <a href="#products" onClick={handleMobileLinkClick} className="text-2xl font-semibold text-[--color-text] hover:text-[--color-primary]">Products</a>
                  <a href="#/gallery" onClick={handleMobileLinkClick} className="text-2xl font-semibold text-[--color-text] hover:text-[--color-primary]">Gallery</a>
                  <a href="#about" onClick={handleMobileLinkClick} className="text-2xl font-semibold text-[--color-text] hover:text-[--color-primary]">About</a>
                  <a href="#contact" onClick={handleMobileLinkClick} className="text-2xl font-semibold text-[--color-text] hover:text-[--color-primary]">Contact</a>
              </div>
          )}
      </div>
    </>
